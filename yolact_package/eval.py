...
def get_color(j, on_gpu=None, class_color=False, undo_transform=True):
    global color_cache
...
def display_masks(image, h, w, masks, nb_masks, mask_alpha=0.45):
    img_numpy = undo_image_transformation(image, w, h)

    img_gpu = torch.Tensor(img_numpy).cuda()

    masks = masks[:nb_masks, :, :, None]

    # Prepare the RGB images for each mask given their color (size [num_dets, h, w, 1])
    colors = torch.cat([get_color(j, on_gpu=img_gpu.device.index).view(1, 1, 1, 3) for j in range(nb_masks)], dim=0)
    ...
    if nb_masks > 1:
        inv_alph_cumul = inv_alph_masks[:(nb_masks-1)].cumprod(dim=0)
...
def prep_display(classes, scores, boxes, masks, img_original, h, w, undo_transform=True, mask_alpha=0.45, score_threshold=0.8, fps_str=''):
    ...
    if num_dets_to_consider > 0:
        img_numpy = display_masks(img_original, h, w, masks, num_dets_to_consider)
        ...
        for j in range(num_dets_to_consider):
            ...
            _classe = cfg.dataset.class_names[classes[j]]
            text_str = '%s: %.2f' % (_classe, score) if args.display_scores else _classe
    ...
    return img_numpy, [(cfg.dataset.class_names[classes[i]], scores[i].item()) for i in range(num_dets_to_consider)]
